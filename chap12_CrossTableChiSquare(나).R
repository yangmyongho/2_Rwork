...
e_rate # 0.3625 = Chi-square contribution

# 교차분할표 -> 카이제곱검정
CrossTable(x=df$Level, y=df$Pass, )
# 검정통계량 =( chi^2 , d.f. ,,,)
# 카이제곱검정통계량 = 각 셀의 기대비율의 합
# 자유도=df : 샘플수(n)에서 자유롭게 선택할수 있는 수
# d.f = (3-1)*(2-1) <(행-1)*(열-1)>
# 카이제곱검정         자유도(검정통계량)    유의확률
# Chi^2 =  2.766951     d.f. =  2            p =  0.2507057 
# 유의확률(0.250) > 유의수준(0.05) 이므로 귀무가설 채택
...
# 귀무가설 : 게임에 적합한 주사위이다. (기각)
# 대립가설 : 게임에 적합하지 않는 주사위이다. (채택)
#-----------------------------------------------
# 가설 설정 방법
# 귀무가설 : 같다 = 다르지않다 = 차이가 없다 = 효과가 없다
# 대립가설 : 같지않다 = 다르다 = 차이가 있다 = 효과가 있다

# 60회 주사위를 던져서 나온 관측도수/기대도수 <대립가설 채택 : 이주사위는 게임에적합하지않다.>
...
# X-squared = 14.2, df(자유도) = 5, p-value(유의확률) = 0.01439
# 유의확률(0.014) < 유의수준(0.05) 이므로 귀무가설 기각
# 만약확률이 이랬다면 
p <- c(0.1, 0.1, 0.25, 0.25, 0.1, 0.2)
chisq.test(c(4,6,17,16,8,9),p=p)
# p-value = 0.789 >= 0.05 이므로 귀무가설 채택
...
# 귀무가설 : 기대치와 관찰치는 차이가 없다. 
# 대립가설 : 기대치와 관찰치는 차이가 있다. 
# 귀무가설 : 스포츠음료의 선호도에 차이가 없다. (기각)
# 대립가설 : 스포츠음료의 선호도에 차이가 있다. (채택)
#-----------------------------------------
data <- textConnection(
  "스포츠음료종류  관측도수
  1   41
...
# 유의확률(0.0003) < 유의수준(0.05) 이므로 귀무가설 기각 
#<유의확률 해석>
...
# 채택역 : p-value >= 알파(0.05)
# 기각역 : p-value < 알파(0.05)
# p-value(0.2507) >= 0.05 -> 귀무가설채택

# <검정통계량 검정>
# d.f. = 2 , 유의수준 : 0.05 -> 임계값(5.991)  < 카이분포표에서 확인가능> 
# 채택역 : chi^2 < 임계값
# 기각역 : chi^2 >= 임계값
# chi^2(2.766) < 임계값(5.991) -> 귀무가설 채택 
