...
length(data$cost)
summary(data$cost) # 요약통계량 - 의미있음(mean) - 8.784 평
mean(data$cost) # NA값때문에 평균이 안구해짐 
data$cost
...
min(x)
max(x)
range(x) # min ~ max 2.1~7.9

sort(x) # 오름차순 
sort(x, decreasing=T) # 내림차순  
# median(x) = sort(x)[124] = 5.4
...
# order : 기존 행 번호 유지
# arrange : 새로운 행 번호 적용
data[sort(data$cost),] # cost값으로 오름차순되는게아님 행순서만오름차순됌 < sort는 벡터단위만가능 > 
data[order(data$cost),] # 행순서는정렬이안되지만 cost기준 오름차순정렬됌 
data[order(data$cost,decreasing = T),] # 내림차순으로 정렬 
library(dplyr)
arrange(data, cost) # 해당 컬럼자체를 행순서까지 cost 기준 오름차순정렬 < index까지 변함>


# 최빈수(Mode) : 빈도수가 가장 높은수
mode() # data type 제공
mode(x) # "numeric"   <최빈수확인불가>
...
# 단봉일경우!! 
# 최빈수 = 중위수 = 평균 -> 좌우대칭
# 최빈수 < 중위수or평균 -> 오른쪽으로기울어짐 
# 최빈수 > 중위수or평균 -> 왼쪽으로기울어짐 
# x 는  5 < 5.4or5.354 -> 오른쪽으로 기울어짐 < x는 단봉이아니지만 우연히 동일한경우>
...
# < 왜도=0 좌우대칭, 왜도<0 오른쪽으로 기울어짐, 왜도>0 왼쪽으로 기울어짐 > 

# 첨도 - 표준정규분포(3) 와 비교하여 얼마나 뽀족한가 측정 지표
kurtosis(cost) # 2.674163  < 3보다 작아서 완만하다 >   

# 기본 히스토그램 
hist(cost)

# 히스토그램 확률밀도/표준정규분포 곡선 
hist(cost, freq = F) # 밀도그래프 <정규분포는 밀도그래프의합이 1이다> 
...
# 정규성 검정
shapiro.test(cost)
# W = 0.98187, p-value = 0.002959  < 0.05  (기각) 
